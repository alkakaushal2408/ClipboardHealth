...
public class LoggerEnricher {
	...
	private static final String ITALIC_STRING = "italic";
	private static final String BOLD_STRING = "bold";

	public enum EnricherFactory {
		RED("red"), GREEN("green"), YELLOW("yellow"), PURPLE("#8241f9"), ORANGE("#ffa500"), BOLD_RED("bold_red"),
		BOLD_GREEN("bold_green"), BOLD_YELLOW("bold_yellow"), BOLD_PURPLE("bold_#8241f9"), ITALIC_RED("italic_red"),
		ITALIC_GREEN("italic_green"), ITALIC_YELLOW("italic_yellow"), ITALIC_PURPLE("italic_#8241f9"),
	...
	public enum TimeZoneFactory {
		ACT("ACT"), AET("AET"), AGT("AGT"), ART("ART"), AST("AST"), BET("BET"), BST("BST"), CAT("CAT"), CNT("CNT"),
		CST("CST"), CTT("CTT"), EAT("EAT"), ECT("ECT"), EST("EST"), HST("HST"), IET("IET"), IST("IST"), JST("JST"),
		MIT("MIT"), MST("MST"), NET("NET"), NST("NST"), PLT("PLT"), PNT("PNT"), PRT("PRT"), PST("PST"), SST("SST"),
		VST("VST");
	...
	/** Builds a string for logging */
	private static String buildFontTag(TimeZone timezone, Object color, Object message) {
		return getTimeStamp(timezone) + " " + "<font color='" + color.toString().toLowerCase() + "'>" + message
				+ "</font>";
	}

	public static synchronized void enrichReporterLog(Object enrichType, Object message) {
		...
		if (enrichType instanceof TimeZone) {
			...
			Reporter.log(buildFontTag(timezone, color, message));
		} else if (enrichType instanceof EnricherFactory) {
			timezone = TimeZone.getTimeZone(tzUTC);

			var enrichValue = EnricherFactory.equalsTo(enrichType);
			...
			if (enrichValue != null) {
				...
				case YELLOW:
				case PURPLE: {
					Reporter.log(buildFontTag(timezone, enrichValue, message));
					break;
				}
				case BOLD_RED:
				case BOLD_GREEN:
				...
				case BOLD_PURPLE: {
					Reporter.log(getTimeStamp(timezone) + " " + "<b><font color='"
							+ enrichType.toString().toLowerCase().split("_")[1] + "'>" + message + "</font></b>");
					break;
				}
				case ITALIC_RED:
				case ITALIC_GREEN:
				...
				case ITALIC_PURPLE: {
					Reporter.log(getTimeStamp(timezone) + " " + "<i><font color='"
							+ enrichType.toString().toLowerCase().split("_")[1] + "'>" + message + "</font></i>");
					break;
				}
				case BOLD: {
			...
			if (enrichType.toString().startsWith("#")) {
				timezone = TimeZone.getTimeZone(tzUTC);
				evaluateFormatAndLog(enrichType, message, timezone);
	...
	private static void evaluateFormatAndLog(Object enrichType, Object message, TimeZone timezone) {
		if ((enrichType.toString().toLowerCase().contains(BOLD_STRING)
				&& enrichType.toString().toLowerCase().contains(ITALIC_STRING))
				|| colorFactoryEquals(enrichType.toString().toLowerCase().replace(BOLD_STRING, "")
						.replace(ITALIC_STRING, "").replaceAll("\\s", "").replace("_", ""))) {
			Reporter.log(getTimeStamp(timezone)
					+ " " + "<b><i><font color='" + enrichType.toString().toLowerCase().replace(BOLD_STRING, "")
							.replace(ITALIC_STRING, "").replaceAll("\\s", "").replace("_", "")
					+ "'>" + message + "</font></i></b>");
		} else if (enrichType.toString().toLowerCase().contains(BOLD_STRING)) {
			Reporter.log(getTimeStamp(timezone) + " " + "<b><font color='" + enrichType.toString().toLowerCase()
					.replace(BOLD_STRING, "").replaceAll("\\s", "").replace("_", "") + "'>" + message + "</font></b>");
		} else if (enrichType.toString().toLowerCase().contains(ITALIC_STRING)) {
			Reporter.log(getTimeStamp(timezone) + " " + "<i><font color='" + enrichType.toString().toLowerCase()
					.replace(ITALIC_STRING, "").replaceAll("\\s", "").replace("_", "") + "'>" + message
					+ "</font></i>");
		} else {
			Reporter.log(buildFontTag(timezone, enrichType, message));
		}
	}

	public static String getTimeStamp(TimeZone timezone) {
		var formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss aa");
